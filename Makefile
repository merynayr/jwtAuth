...
PKG=./tests
COVERPKG=./internal/...
COVER_TMP=coverage.tmp.out
COVER_FINAL=coverage.out

test:
	go clean -testcache
	go test $(PKG) -cover -covermode=count -coverpkg=$(COVERPKG) -count=5

test-coverage:
	go clean -testcache
	go test $(PKG) -coverprofile=$(COVER_TMP) -coverpkg=$(COVERPKG) -covermode=count -count=5
	grep -v 'mocks\|config' $(COVER_TMP) > $(COVER_FINAL)
	rm $(COVER_TMP)
	@go tool cover -html=$(COVER_FINAL)
	@grep -sqFx "/$(COVER_FINAL)" .gitignore || echo "/$(COVER_FINAL)" >> .gitignore
	@go tool cover -func=$(COVER_FINAL) | grep -E "total|CreateUser"
